...
NS_LOG_COMPONENT_DEFINE ("DctcpCase1");
...
main (int argc, char *argv[])
{
  ...
  if (writeForPlot)
    {
      filePlotQueue << pathOut << "/" << "red-queued.plotme";
      filePlotQueueAvg << pathOut << "/" << "red-queue_avgd.plotme";
  ...
  if (writeThroughput)
    {
      for (auto& resultList : throughputResult)
        {
          std::stringstream ss;
          ss << pathOut << "/throughputd-" << resultList.first << ".plotme";
