...
NS_LOG_COMPONENT_DEFINE ("tcptests");

// attributes
DataRate link_data_rate;
Time link_delay;
uint32_t short_flow_num = 20;
uint32_t short_flow_size = 6000;
  ...
  if (writeThroughput)
    {
      for (auto& resultList : throughputResult)
        {
          std::stringstream ss;
          ss << pathOut << "/throughput-" << resultList.first << ".plotme";
